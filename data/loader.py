...
def load_salaries(historical=False):
            ...
            for week in weeks:
                for plat in platforms:
                    ...
                    if platform_df is not None:
                        ...
                        platform_df['contest_id'] = exp.contest_search(plat, 'all week')
    ...
    for ix, row in all_data.iterrows():
        ...
        row_df['contestid'] = row['contest_id']
        ...
        for ix, row in players_df.iterrows():
            ...
            if team:
                gamelog_df['gameid'] = gamelog_df.apply(lambda x: exp.game_insert(x.boxscore_word_link, x.timeid,
                                                                                  x.hometeamid, x.awayteamid), axis=1)
            else:
            ...
            if not team:
                ...
                stacked_df = gamelog_df.set_index(['gameid', 'playerid']).T.ix[stats].T.astype(
                                                    float).stack().dropna().reset_index()
...
def load_projections(historical=False):
    if historical:
        ...
        for year in years:
            for week in weeks:
                platform_df = fp.get_projections(year, week)
                if platform_df is not None:
                    platform_df['timeid'] = exp.time_search(year, week)
                    platform_df['source'] = 'fantasypros'
                    all_data = all_data.append(platform_df, ignore_index=True)
    else:
        1 + 1
    all_data['playerid'] = all_data.apply(lambda x: exp.player_search(x.PLAYER, dst_mode=(x.position == 'DST'),
                                        filters={'position': x.position, 'year': x.year, 'team': x.TEAM},
                                        auto_insert=True, source='fantasypros', sourceid=x.SOURCE), axis=1)
    stats = list(set(sum(fp.positions.values(), [])))
    stacked_df = all_data.set_index(['timeid', 'playerid', 'source']).T.ix[stats].T.stack().dropna().reset_index()
    stacked_df.columns = ['timeid', 'playerid', 'source', 'stat', 'value']
